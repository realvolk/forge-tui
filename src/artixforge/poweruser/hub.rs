...
pub fn run(
    ...
    let result = loop {
        ...
        term.draw(|f: &mut Frame| {
            ...
            let box_area = layout::centered(90, 92, area);
            ...
            let item_lines: Vec<Line> = visible_items.iter().enumerate().map(|(i, item)| {
                ...
                let display_val = if item.display == "flag_list" {
                    if val.is_empty() { "none".into() } else { val }
                } else if item.display == "profile_preview" {
                    format!("{} (CFLAGS: see preview)", val)
                } else if val.is_empty() { "(not set)".into() } else { val };
                Line::from(vec![
            ...
            // Footer
            let footer = " j/k:nav  Tab:switch  Enter:edit  F1:Build  Esc:cancel";
            f.render_widget(
            ...
            if let Some(item) = visible_items.get(state.item_idx).cloned() {
                ...
                let result = match item.widget.as_str() {
                    ...
                    "recipe_sections" => {
                        ...
                        let resp = widgets::checklist::run(Some(term), "Recipe Sources".into(),
                            "Select sections:".into(), sections.iter().map(|s| s.to_string()).collect(),
                            None, None, None, None)?;
                        if resp.cancelled { Ok::<Option<String>, anyhow::Error>(None) } else {
                            Ok(resp.result.and_then(|v| v.as_array().cloned())
                                .map(|a| a.iter().filter_map(|v| v.as_str()).collect::<Vec<_>>().join(" ")))
